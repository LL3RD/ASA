...
def evaluate_BraTS_case(arr: np.ndarray, arr_gt: np.ndarray):
    ...
    # tumor core
    mask_gt = (arr_gt == 2).astype(int)
    mask_pred = (arr == 2).astype(int)
...
def evaluate_BraTS_folder(folder_pred, folder_gt, num_processes: int = 24, strict=False):
    ...
    with open(join(folder_pred, 'results.csv'), 'w') as f:
        f.write("name,DC_CSF,DC_GM,DC_WM,HD95_CSF,HD95_GM,HD95_WM\n")
...
def main():
    folder_pred = '/data2/huangjunjia/nnFormer/nnFormer_trained_models/nnFormer/3d_fullres/Task666_IBSR/Paper/infers/IBSR_Jig_Final'
    evaluate_BraTS_folder(folder_pred,
                          '/data2/huangjunjia/nnFormer/nnFormer_raw/nnFormer_raw_data/Task666_IBSR/labelsTs/')

    result = pd.read_csv(join(folder_pred, 'results.csv'))[
        ['DC_CSF', 'DC_GM', 'DC_WM', 'HD95_CSF', 'HD95_GM', 'HD95_WM']]
