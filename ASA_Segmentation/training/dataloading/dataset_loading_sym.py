...
class DataLoader3D_SYM(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
        ...
        """
        super(DataLoader3D_SYM, self).__init__(data, batch_size, None)
        ...
        self.pos_emb = self.create_pos_emb(SET=SET)
    ...
    def generate_train_batch(self):
        ...
        # seg = np.concatenate((seg, self.pos_emb.copy()), axis=1)

        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys,
                'pos_tr': self.pos_emb.copy()}

    def create_pos_emb(self, SET):
        if SET == "TRAIN":
            shape2 = (2, 1, 200, 200, 200)
            pos_emb = np.zeros(self.seg_shape, dtype=np.float32)
            img = np.zeros(shape2, dtype=np.float32)
            for u in range(pos_emb.shape[0]):
                cnt = 1
                for i in range(25):
                    for j in range(25):
                        for k in range(25):
                            if k <= 12:
                                patch = np.ones((8, 8, 8), dtype=np.float32) * cnt
                                rec = cnt
                                cnt += 1
                            else:
                                rec -= 1
                                patch = np.ones((8, 8, 8), dtype=np.float32) * rec
                            img[u, :, i * 8:(i + 1) * 8, j * 8:(j + 1) * 8, k * 8:(k + 1) * 8] = patch
            for u in range(pos_emb.shape[0]):
                pos_emb[u, 0] = np.pad(img[u, 0, :, :, :], ((2, 3), (2, 3), (2, 3)), 'constant')
        else:
            pos_emb = np.zeros(self.seg_shape, dtype=np.float32)
            for u in range(pos_emb.shape[0]):
                cnt = 1
                    ...
                    for j in range(16):
                        for k in range(16):
                            if k <= 7:
                                ...
                                patch = np.ones((8, 8, 8), dtype=np.float32) * rec
                                rec -= 1
                            pos_emb[u, :, i * 8:(i + 1) * 8, j * 8:(j + 1) * 8, k * 8:(k + 1) * 8] = patch
...
if __name__ == "__main__":
    ...
    # axes_len = {f"p{i + 1}": p for i, p in enumerate((8, 8, 8))}
    # embeddings = nn.Sequential(
    #     Rearrange(f"{from_chars} -> {to_chars}", **axes_len),
    # )
    # emb = embeddings(torch.from_numpy(tes))
    # pos_embedding = torch.zeros((2, 4096, 384))
    # # emb_pos = torch.zeros(emb.shape[1])
    # emb_pos = torch.mode(emb, dim=2).values
    # emb_pos_sort, index = torch.sort(emb_pos, dim=1)
    #
    # for b in range(emb_pos.shape[0]):
    #     cnt = -1
    #     value = None
    #     for e in range(emb_pos.shape[1]):
    #         if emb_pos_sort[b, e] != value:
    #             cnt += 1
    #             pos_embedding[b, index[b, e], :] = POS[cnt, :]
    #             value = emb_pos_sort[b, e]
    #         else:
    #             pos_embedding[b, index[b, e], :] = POS[cnt, :]
    #
    # print(pos_embedding)
    ...
    dataset = load_dataset(
        "/data2/huangjunjia/nnFormer/nnFormer_preprocessed/Task999_BraTS2021/nnFormerData_plans_v2.1_stage0/")
    unpack_dataset(
        "/data2/huangjunjia/nnFormer/nnFormer_preprocessed/Task999_BraTS2021/nnFormerData_plans_v2.1_stage0/")
    dl = DataLoader3D_SYM(dataset, (205, 205, 205), (128, 128, 128), 2, oversample_foreground_percent=0.33)

    # x = dl.generate_train_batch()
    # import matplotlib.pyplot as plt

    # def normalization(data):
    #     _range = np.max(data) - np.min(data)
    #     return (data - np.min(data)) / _range
    # print(normalization(x['seg'][0, 1, 100, :, :][3:-3, 3:-3])[30:100, 30:100])
    # plt.imshow(normalization(x['seg'][0, 1, :, :, 100][3:-3, 3:-3]), cmap='gray')
    # plt.show()
    # # print(x['seg'].shape)
    # # print(x['seg'][0, 1, 100, 100, :])
    from nnformer.training.data_augmentation.data_augmentation_moreDA_SYM import get_moreDA_augmentation_sym
    ...
    def setup_DA_params():
        net_num_pool_op_kernel_sizes = np.array([[2, 2, 2], [2, 2, 2], [2, 2, 2], [2, 2, 2]])
        ...
        data_aug_params['deep_supervision_scales'] = deep_supervision_scales
        ...
        data_aug_params["mask_was_used_for_normalization"] = OrderedDict([(0, True), (1, True), (2, True), (3, True)])
        ...
        data_aug_params['selected_seg_channels'] = None
        data_aug_params['patch_size_for_spatialtransform'] = patch_size_for_spatialtransform

        data_aug_params["num_cached_per_thread"] = 4
        data_aug_params["num_threads"] = 2

        return data_aug_params


    params = setup_DA_params()
    dl_aug, _ = get_moreDA_augmentation(dl, dl, params['patch_size_for_spatialtransform'],
                                        params,
                                        deep_supervision_scales=params['deep_supervision_scales'],
                                        pin_memory=True,
