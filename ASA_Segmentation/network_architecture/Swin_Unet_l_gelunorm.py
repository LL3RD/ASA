...
class Patch_Expanding(nn.Module):
    def __init__(self, dim, norm_layer=nn.LayerNorm, up_padding=(0, 0, 0)):
        super().__init__()
        self.dim = dim
        self.norm = norm_layer(dim)
        self.up = nn.ConvTranspose3d(dim, dim // 2, 2, 2, output_padding=up_padding)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, S, H, W):
        ...
        Sp = int(np.ceil(S / self.window_size)) * self.window_size # 向上取整
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self,
        ...
        self.Upsample = upsample(dim=2 * dim, norm_layer=norm_layer, up_padding=up_padding)
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers)[::-1]:
            layer = BasicLayer_up(
                ...
                up_padding=(0, 0, 1) if i_layer == 0 else (0, 0, 0),
            )
            self.layers.append(layer)
...
class swintransformer(SegmentationNetwork):
    ...
    def __init__(self, input_channels, num_classes, deep_supervision=True):
        ...
        patch_size = [4, 4, 4]
        window_size = 4
        self.model_down = SwinTransformer(pretrain_img_size=[128, 160, 112], window_size=window_size,
        ...
        self.encoder = encoder(pretrain_img_size=[128, 160, 112], embed_dim=embed_dim, window_size=window_size,
...
if __name__ == '__main__':
    device = "cuda:7"
    model = swintransformer(input_channels=4, num_classes=4, deep_supervision=True).to(device)
    # model = VisionTransformer(img_size=[128, 128, 128], patch_size=[8, 8, 8], in_chans=1, adaptor=False).to("cuda:3")
    x = torch.rand((2, 4, 128, 160, 112)).to(device)
    y = model(x)
    print(len(y))
    for i in y:
        print(i.shape)
