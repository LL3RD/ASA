...
def get_default_configuration(network, task, network_trainer, plans_identifier=default_plans_identifier,
                              ...
                              base_module='nnformer.training.network_training', tag='default'):
    ...
    elif task == 'Task555_WMH':
        plans['plans_per_stage'][0]['batch_size'] = 2
        plans['plans_per_stage'][0]['patch_size'] = np.array([48, 128, 128])
        pickle_file = open(plans_file, 'wb')
        pickle.dump(plans, pickle_file)
        pickle_file.close()
    ...
    output_folder_name = join(network_training_output_dir, network, task, "Paper", network_trainer + "__" + tag)
