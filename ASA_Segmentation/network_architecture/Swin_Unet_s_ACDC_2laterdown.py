...
class nnFormer(SegmentationNetwork):
    ...
    def forward(self, x):
        ...
        if self._deep_supervision and self.do_ds:
            ...
            seg_outputs = self.final[0](out[-1])
...
if __name__ == '__main__':
    device = "cuda:7"
    model = nnFormer(input_channels=2, num_classes=1, crop_size=[64,128,128],  deep_supervision=True).to(device)
    # model = VisionTransformer(img_size=[128, 128, 128], patch_size=[8, 8, 8], in_chans=1, adaptor=False).to("cuda:3")
    x = torch.rand((2, 2, 64, 128, 128)).to(device)
    y = model(x)
    print(len(y))
    for i in y:
        print(i.shape)
